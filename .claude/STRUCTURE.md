...
│   ├── TicketKey.cs          # THE ticket-key regex: IsValid / Normalize / Require (shared by every writer)
│   └── Repositories/
│       ├── SessionRepo.cs
│       ├── TicketRepo.cs
...
│   ├── TicketKeyTests.cs         # Data/TicketKey validator (shape + Require throwing)
│   ├── JiraSiteUrlTests.cs       # https-only site URL, normalization, host-change → clear token
│   ├── ClaudeCommandTests.cs     # typed-command safety: quote breakout, control chars, caps, shape
...
| `Bridge/Handlers/SettingsHandlers.cs` | Actions `settings.get` (never returns the token value — write-only; also returns `jiraSiteUrlInsecure` when a stored site URL isn't https), `settings.set` (routes token to `SetProtected`; **validates `jiraSiteUrl` through `JiraSiteUrl.Normalize` before writing anything** and clears `jira_token` when the host changes, returning `{tokenCleared}`; on allowlist change calls `PurgeDisallowedAutoLinks`). `PurgeDisallowedAutoLinks()` removes auto links whose key is outside the allowlist, keeping manual/confirmed links, and cleans orphan tickets. |
...
| `js/views/settings.js` | Settings form: JIRA site URL (`type="url"`, https required — a warning banner appears when `jiraSiteUrlInsecure`, and `save()` toasts when a host change cleared the stored token), email/token (write-only), scan paths, allowlist, backfill date, fetch JQL; Test connection. Note: an invalid stored site URL makes `settings.set` reject the whole payload, so it must be fixed before other settings can be saved from any panel. |
...
| `TicketKeyTests.cs` | `Data/TicketKey` — accepted/rejected key shapes, `Normalize`, `Require` throwing with the UI message. |
| `ClaudeCommandTests.cs` | The typed `claude …` line: PowerShell/bash quote-breakout payloads for every member of the single-quote class, control-character stripping, length caps, model/permission allowlists, session-id rejection, untrusted-description fencing, and the ordinary-input command shape. |
