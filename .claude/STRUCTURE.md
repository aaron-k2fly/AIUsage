...
| `Terminal/AgentCatalog.cs` | `List(projectDir, customDir)` → agents from the custom dir + `<projectDir>/.claude/agents` + `~/.claude/agents` (name/description frontmatter; first-seen wins). `SyncCustomAgents(customDir, workingFolder)` copies the custom folder's `*.md` into the working folder's `.claude/agents` before a session (no clobber) so `--agent` resolves them. |
