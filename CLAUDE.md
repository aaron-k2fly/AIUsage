...
dotnet run -- --route <page>            # open the app directly on a page (dashboard|sessions|manual|tickets|settings) — used for headless UI verification
...
There is no test project — verification is done via the `--scan`/`--sql`/`--route` CLI verbs and window screenshots.
